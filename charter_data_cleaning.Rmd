...
```{r}
dfp$ncessch %>% unique %>% length()
dfp$year %>% unname

as.numeric(levels(dfp$year))
matrix(data = c(dfp$year %>% unname,dfp$year %>% as.numeric),ncol=2)
```

Fixed effects regression in R
```{r}
# init
dmax_set = c(20,40,60,80,100)[1]; a_set = c(0.05,0.25,1,5,25)[1]; depvar_set = "combined_proficiency"; 
lags_set = combinations(7,2,c(0:6)); lags_set = paste(lags_set[,1],lags_set[,2],sep=":"); lags_set = "0:6"
regs = data.table(dmax = numeric(), a = numeric(),lags = character(),rsquared = numeric(),depvar = character())
...
# 2-3 seconds for 1 iteration
for (h in c(100)) {
    tic()
    # iterate through all fixed-effects regressions
    for (dmax in dmax_set) {
      for (a in a_set) {
        for (depvar in depvar_set) {
          for (lags in lags_set) {
                r <- plm(as.formula(paste(depvar," ~ year + as.numeric(year) * as.factor(ncessch) + lag(csp_a",a,"_dmax",dmax,",",lags,")",sep="")), 
                  data=dfp %>% head(h), model="within")
                regs <- rbind(regs,list(dmax,a,lags,r.squared(r),depvar))      
          }
        }
      }
    }
