...
# LOOP ----
# iterate through hyperparameters r
for (r in c(-1)) { 
  print(r)
...
a_set = c(1,.25,.05,5,25)#c(0.05,0.25,1,5,25)
...
for (dmax in dmax_set) { 
  ...
  for (a in a_set) {
    ...
    for (y in 1999:2018) {
      ...
      sp <- data.table(dmf %*% y_enr[[y]]); setnames(sp,paste0('sp_',grades))
      # CHARTER STUDENT PRESENCE proximate to location I, weighted by distance curving function
      csp <- data.table(dmf %*% y_cenr[[y]]); setnames(csp,paste0('csp_',grades))
...
Validation
```{r}
t_lids[,.N,by=.(location_id,year)][N>1]
select(t_m,year,location_id,paste0("csp","_a",a,"_dmax",dmax))[,.N,by=.(location_id,year)][N>3]

merge(df[,.N,by=.(ncessc,year)][N>3],t_m,by=c("location_id","year"))      #[,.N,by=.(location_id,year)][N>3]



if (setdiff(unique(df$location_id),c(1:nrow(dm)))) print("DF and DM do not have matching location_id")
```


```{r}
# plot the current results of the distance function
plot(dm[1,],dmf[1,],xlim=c(0,10))
...
```{r}
# library(ggplot2)
lid = sample(df$location_id,1); rad = 5; y = 2018
lid = 13
colorset = (which(dm[lid,]<rad) %>% unname)

# plot it 
ggplot(df) + 
  coord_fixed() + 
  geom_point(aes(x=gm_lon,y=gm_lat,color=location_id%in%colorset),size=0.01) + 
  geom_point(data = df[location_id == lid],aes(x=gm_lon,y=gm_lat),size = 0.2, color = "black")

# view all rows in year with lid
df[year == y & location_id %in% colorset] %>% select(grades,everything()) %>% view
...
```{r}
dfp <- pdata.frame(df,index=c("ncessch","year"))
test <- plm(math~teacher_student_ratio,data=dfp,model="within")
predict(test)
str(test)
test$coefficients
test %>% str
