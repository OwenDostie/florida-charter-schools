...
```{r}
# init
dmax_set = c(20,40,60,80,100); a_set = c(0.05,0.25,1,5,25)
...
# test FE regression
r <- plm(comb_math_proficiency~csp_a1_dmax20,data=dfp,model="within"); r

# test FE regression
r <- plm(diff(comb_math_proficiency)~csp_a1_dmax20,data=dfp,model="within"); r

# test FE regression
r <- plm(diff(comb_math_proficiency)~csp_a1_dmax20 + year,
         data=dfp, model="within"); r

# full regression
r <- plm(diff(comb_math_proficiency,1)~diff(comb_math_proficiency,lag=c(1:6)) + diff(csp_a1_dmax20,lag=c(0:6)) + year,
         data=dfp, model="within"); r
...
# iterate through all fixed-effects regressions
for (c in paste0("csp_a",a_set,paste0("_dmax",dmax_set))) {
  r <- plm(comb_math_proficiency~get(c),data=dfp,model="within")
  print(c)
  print(r)
}
