...
```{r}
a = 0.05; dmax = 20; depvar = "combined_proficiency"

r <- plm(as.formula(paste(depvar," ~ year + character(ncessch) +lag(csp_a",a,"_dmax",dmax,",",lags,")",sep="")), 
  ...
  for (a in a_set) {
    for (depvar in depvar_set) {
      for (lags in lags_set) {
            r <- plm(as.formula(paste(depvar," ~ year + lag(csp_a",a,"_dmax",dmax,",",lags,")",sep="")), 
              data=dfp, model="within")
